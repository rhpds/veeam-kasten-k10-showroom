...
. Select *_Virtualization → Virtual Machines_* from the sidebar, and click *_Create VirtualMachine -> From Instance Type_* in the `kasten-lab-{user}` Project.
+
...
Kasten can be used to protect VMs provisioned using a `Template` as well without any additional configuration.
====

. Under *_Select Volume to boot from_*, select the `centos-stream9` volume.
+
====
[NOTE]

If you do not see any bootable volumes, ensure you've selected `openshift-virtualization-os-images` for the *_Volumes Project_*
====
+
image::module01-lab03-backup_restore/04.png[]
. Leave the default *_General Purpose_* InstanceType selected
. Update the *_VirtualMachine name_* to `{user}-fedora` and click *_Create VirtualMachine_*.
+
...
. Validate the `{user}-fedora-volume` PersistentVolumeClaim configuration from *_Storage → PersistentVolumeClaims_* in the sidebar.
+
...
. In the *_Web Terminal_*, enable filesystem freezing for `{user}-fedora`:
+
[source,bash,role=execute,subs="attributes"]
----
oc annotate virtualmachine {user}-fedora \
  -n kasten-lab-{user} \
...
====
[NOTE]
The above annotation can be managed via the Kasten web UI by navigating to the *_Virtual Machines_* Dashboard, selecting the checkbox next to the VMs you wish to enable, and selecting *_Enable Guest OS Freeze_*. The freeze and unfreeze operations will only be attempted if the VirtualMachine is in *_Running_* state.
 
image::module01-lab03-backup_restore/14a.png[]
...
. Select the most recent RestorePoint of type *_Snapshot_*, and click the local version as shown below.
+
...
. Once the *_Restore_* has completed, return to *_OpenShift Console → Virtualization → Virtual Machines_* and validate the `{user}-fedora` VM is *_Running_*.
+
...
[source,bash,role=execute,subs="attributes"]
----
oc describe pvc {user}-fedora -n kasten-lab-{user}
----
...
[source,bash,role=execute,subs="attributes"]
----
oc delete virtualmachine {user}-fedora -n kasten-lab-{user}
...
[source,bash,role=execute,subs="attributes"]
----
oc describe pvc {user}-fedora -n kasten-lab-clone-{user}
----
====
