...
While you do have access to the `kasten-io` namespace *PLEASE DO NOT MODIFY ANYTHING IN THAT NAMESPACE*. This namespace is shared by all lab participants and changing anything could break the lab for *everyone*!
 +
An dedicated namespace has been precreated for you named, *_backuptarget1-{user}_*
====

As Kasten has been pre-deployed on the cluster, it can be used to take a local snapshot of an OpenShift application or VM using Kasten - _but snapshots are not backup!_ In order to restore in the event the local cluster or primary storage is compromised, a copy of that data should be exported to another location.
...
. In the *_Kasten Dashboard_*, select *_Profiles → Location_* from the sidebar and click *_+ Create New Profile_*.
+
...
While we won't be using immutabile buckets for this lab, in a production environment this is something you'd certainly want to use, to ensure
your backup data is safe from accidental deletion or ransomware attack!
====
