...
. In the *_OpenShift Console_*, search for `Kasten` in the *_OperatorHub_* and select *_Veeam Kasten_*:
+
image::module01-lab01-install/02.png[]
+

. Under *_Version_*, select `{k10_version}` from the dropdown menu, and click *_Install_*.
+
...
. Click *_Create_*.
. Ensure you are in your user's namespace, *_backuptarget-{user}_*. Open the *_Web Terminal_* and click *_Start_* to initialize the terminal (if prompted).
+
